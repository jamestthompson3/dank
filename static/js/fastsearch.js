  ...
  if (results.length === 0) {
   ...
   // build our html
    for (let item of iterator.slice(0, 5)) {
      ...
      searchitems = searchitems.concat(`<li><a href="${item.permalink}" tabindex="0"><span class="title">${item.title}</span><br /> <span class="sc">${item.section}</span> — ${item.date} — <em>${item.desc}</em></a></li>`);
    }
